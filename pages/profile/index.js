...
const Profile = () => {
  ...
  const [professionText, setProfessionText] = useState("Web Application developer")
  const user = {
  ...
  return (
    <div className='single_user_profile_section text_no_select'>
        ...
        <div className="profile_wrapper grid grid-cols-12 gap-5">
          ...
          <div className="col-span-12 xl:col-span-8">
            ...
            <div className='shadow-md flex'>
              <div style={profileSelectableCard} className={`w-4/12 flex items-center justify-center flex-col py-2 border-b-2 cursor-pointer ${tabs === "about" ? "bg-[#c3d9ec] text-[#c3d9ec] border-[#3980c0]" : "border-gray-500 bg-50"}`} onClick={() => setTabs("about")}>
                <span className='text-xl'>
                  <GrUserManager />
                </span>
                <span className={`text-base font-medium ${tabs === "about" ? "text-[#3980c0]" : "text-gray-800"}`}>About</span>
              </div>
              <div style={profileSelectableCard} className={`w-4/12 flex items-center justify-center flex-col py-2 border-b-2 cursor-pointer ${tabs === "task" ? "bg-[#c3d9ec] text-[#c3d9ec] border-[#3980c0]" : "border-gray-500 bg-50"}`} onClick={() => setTabs("task")}>
                <span className='text-xl text-gray-800'>
                  <GrUserManager />
                </span>
                <span className={`text-base font-medium ${tabs === "task" ? "text-[#3980c0]" : "text-gray-800"}`}>Tasks</span>
              </div>
              <div style={profileSelectableCard} className={`w-4/12 flex items-center justify-center flex-col py-2 border-b-2 cursor-pointer ${tabs === "review" ? "bg-[#c3d9ec] text-[#c3d9ec] border-[#3980c0]" : "border-gray-500 bg-50"}`} onClick={() => setTabs("review")}>
                <span className='text-xl text-gray-800'>
                  <GrUserManager />
                </span>
              ...
              <div className='p-3'>
                <div className='relative -top-16'>
                  <div className='w-20 h-20 rounded-full border-4 border-white overflow-hidden absolute'>
                    ...
                    <img className='w-full hover:scale-105 h-full transition-all duration-300 ease-linear' src={avatar} alt="" />
                  </div>
                </div>
                <div className='pt-4'>
                  <strong className="block text-xl text-gray-700">{name}</strong>
                  {state.editor === "PROFESSION_EDIT" ? (
                    <Input placeholder="input with clear icon" allowClear onChange={e => setProfessionText(e)} />
                  ) : (<span className="block text-lg font-normal" >{professionText} <span className="text-lg pl-3 cursor-pointer text-[#e83a3b]" onClick={() => dispatch({ type: "PROFESSION_EDIT" })}><FiEdit /></span></span>)}
