...
const JobSearch = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs/jobs`,
      )
      .then(
        (response) => {
      ...
      <main className="min-h-[70vh]">
            ...
            <section className="container mx-auto px-4 py-8 min-h-[60vh]">
              ...
              >
                {jobs?.filter(value => {
                  if (searchTerm === "") {
                    return value;
                  } else if (value.jobTtile.toLowerCase().includes(searchTerm.toLowerCase())) {
                    return value;
                  }
                })?.map((job, i) => (
                  <div
                  ...
                  >
                    <div>
                      <img className="max-w-[120px]" src={job?.imgUrl} alt="" />
                    </div>
                    <div>
        ...
        {preview && (
          <div className="bg-gray-50">
            <button className="py-3 px-8 bg-black text-white" onClick={() => setPreview(null)}>Back</button>
            <section style={{ boxShadow: "-2px 3px 15px rgba(0,0,0,0.05)" }} className="container bg-white mx-auto p-8">
              <div className="flex justify-between">
                <div>
                  <h1 className="text-3xl">{preview?.jobTtile}</h1>
                  <div className="flex mt-4 items-center">
                    <div>
                      <img className="w-16 h-16 rounded-xl mr-3" src={preview?.imgUrl} alt="" />
                    </div>
                    <div>
                      <h3 className="text-xl font-normal font-sans">{preview?.companyName}</h3>
                ...
                <div className="flex">
                  <div className="flex mr-4 -mt-1 ">
                    <span className="mt-1 mr-2 text-xl text-gray-400 hover:text-black duration-200 cursor-pointer"><BsBookmark /></span><span className="text-xl text-gray-400">SAVE</span>
                  </div>
                  <i className="text-xl text-gray-400 hover:text-black duration-200 cursor-pointer"><FaShareAlt /></i>
                </div>
              </div>
              <div>
                <hr className="my-6 w-full" />
                <button onClick={() => handleClickJob()} className="bg-[#1a73e8] py-2 rounded-md px-6 text-white">Easy Apply</button>
                <button onClick={() => Router.push("https://www.google.com/search?q=developers+jobs&oq=developers+jobs&aqs=chrome..69i57j0i22i30l2j0i10i22i30l2j0i22i30j0i15i22i30l2j0i10i15i22i30j0i15i22i30.5472j1j15&sourceid=chrome&ie=UTF-8&ibp=htl;jobs&sa=X&ved=2ahUKEwiK75yB--r3AhXk6nMBHbx7BkcQutcGKAF6BAgJEAY&sxsrf=ALiCzsZAA_EBYyCQSBkWiSP6uuUU3Y1YSg:1652942594067#htivrt=jobs&htidocid=jRihaQaAQ6UAAAAAAAAAAA%3D%3D&fpstate=tldetail")} className="bg-[#1a73e8] py-2 rounded-md px-6 text-white ml-3">Apply on {preview?.companyName}</button>
                <hr className="my-6 w-full" />
              </div>
              <div>
                <h2 className="text-xl ">About this job:</h2>
                <p className="p-4 text-justify">{preview?.companyDescription}</p>
              </div>
              <div>
